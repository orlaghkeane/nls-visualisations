...
def clean_creator_name(name):
    ...
    # Add a full stop to middle names
...
# Use creator name to see if they are author
...
def get_author_info_from_wikipedia(cleaned_creator):
    ...
    occupation_element = soup.find('td', {'class': 'infobox-data role'})

    # Check if the author has an occupation listed
    if occupation_element:
        occupation = occupation_element.text.strip()
        return cleaned_creator, occupation
...
for index, row in df.head(100).iterrows():
    ...
    cleaned_creator = clean_creator_name(row['creator'])
    
    # Print the cleaned creator name
    print(f"Cleaned Creator Name: {cleaned_creator}")
    
    # Get author info from Wikipedia
    author_name, occupation = get_author_info_from_wikipedia(cleaned_creator)
    
    # Print the author name and occupation
